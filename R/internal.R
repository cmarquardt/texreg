...
coeftostring <- function(x, lead.zero = FALSE, digits = 2, scientific = NULL) {
  if (!is.finite(x)) {
    return("")
  }
  if (digits < 0) {
  ...
  if (! is.null(scientific)) {
    y <- format(x, digits = digits, scientific = scientific)
  } else {
        ...
        if (is.na(m[i, j])) {
          ...
          if (ci[k - 1] == FALSE) {
            std <- paste(se.prefix.current, coeftostring(m[i, j + 1],
                leading.zero, digits = digits, scientific = scientific), se.suffix.current, sep = "")
          } else {
            std <- paste(se.prefix.current, coeftostring(m[i, j + 1],
                leading.zero, digits = digits, scientific = scientific), semicolon,
                coeftostring(m[i, j + 2], leading.zero, digits = digits, scientific = scientific),
          ...
          entry <- paste(dollar, bold.pref, coeftostring(m[i, j], leading.zero,
              digits = digits, scientific = scientific), bold.suff, std, p, dollar, sep = "")
      ...
      while (j <= length(m)) {
        if (is.na(m[i, j]) || is.nan(m[i, j])) {
          ...
          output.matrix[(i * 2) - 1, k] <- paste(dollar, bold.pref,
              coeftostring(m[i, j], leading.zero, digits = digits, scientific = scientific), bold.suff,
              p, dollar, sep = "")
          if (ci[k - 1] == FALSE) {
            output.matrix[(i * 2), k] <- paste(dollar, se.prefix.current,
                coeftostring(m[i, j + 1], leading.zero, digits = digits, scientific = scientific),
                se.suffix.current, dollar, sep = "")
          } else {
            output.matrix[(i * 2), k] <- paste(dollar, se.prefix.current,
                coeftostring(m[i, j + 1], leading.zero, digits = digits, scientific = scientific),
                semicolon, coeftostring(m[i, j + 2], leading.zero,
                digits = digits, scientific = scientific), se.suffix.current, dollar, sep = "")
          }
        }
        k <- k + 1
        j <- j + 3
    ...
    for (i in 1:length(gofs[, 1])) {
      ...
      for (j in 1:length(gofs[1, ])) {
        strg <- coeftostring(gofs[i, j], leading.zero,
            digits = decimal.matrix[i, j], scientific = scientific)
